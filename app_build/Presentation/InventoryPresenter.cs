...
namespace Shinzui.Presentation.Inventory
{
    public class InventoryPresenter : IInitializable, ITickable, IDisposable
    {
        ...
        public void Initialize()
        {
            ...
            // 1. 開閉状態の同期 (UseCase.IsOpen -> View.Show/Hide & Input Block)
            _useCase.IsOpen
                .Subscribe(isOpen =>
                {
                    if (isOpen)
        ...
        public void Tick()
        {
            ...
            // インベントリが開いている場合のみEキー（使用キー）の入力を検出
            if (_useCase.IsOpen.Value && _inputService.ItemUsePressed)
            {
