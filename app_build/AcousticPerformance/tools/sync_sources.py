...
subprocess.run([sys.executable,str(root/'app_build/Propagation/tools/sync_sources.py')],check=True)
def meta(path):
...
for layer in ('Runtime','Editor'):
    target=root/'Assets/Shinzui/Tests/AcousticPerformance'/layer;target.mkdir(parents=True,exist_ok=True)
    current=target
    while current!=root/'Assets':meta(current);current=current.parent
    for file in (root/'app_build/AcousticPerformance/Tests'/layer).iterdir():
        destination=target/file.name;destination.write_bytes(file.read_bytes());meta(destination)
print('AcousticPerformance test sources synchronized.')
