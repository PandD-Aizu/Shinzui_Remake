    ...
    public class UnitySettingsApplier : ISettingsApplier
    {
        ...
        public void ApplyAudio(Domain.Settings.AudioSettings audio)
        {
            try
            {
                ...
                {
                    // サービスが利用できない場合は従来通りBusに直接設定（フォールバック）
                ...
                // (FMODの特定のスナップショットを有効化/無効化するなどで制御するのが一般的です)
            }
            catch (Exception ex)
            {
        ...
        public void ApplyGraphics(GraphicsSettings graphics)
        {
            ...
            // 3. テクスチャ解像度 (0:高, 1:中, 2:低, 3:超低) ※UnityのglobalTextureMipmapLimitは逆値
        ...
        {
            // カメラワークの挙動（Cinemachineの入力感度や軸反転等）を制御するマネージャーへパラメータを通知
        ...
        public void ApplyLanguage(LanguageSettings language)
        {
            ...
            // 例: LocalizationSettings.SelectedLocale = LocalizationSettings.AvailableLocales.GetLocale(language.DisplayLanguage);
        }

        public void ApplyAccessibility(AccessibilitySettings accessibility)
        {
            // 字幕表示システム（文字サイズ、背景透過度、中心ドット等）にパラメータを反映
        }
