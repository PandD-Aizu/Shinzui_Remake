...
namespace Shinzui.Src.Title
{
    ...
    public class CreditData : UnityEngine.ScriptableObject
    {
        ...
        /// <summary>Snapshot authoring data so presenters never depend on this asset.</summary>
        public CreditContent CreateContent(bool spaceBetweenSections)
        {
            var text = new StringBuilder();
            text.AppendLine();
            text.AppendLine($"<b>{gameTitle}</b>");
            text.AppendLine();
            if (creditSections != null)
            {
                foreach (var section in creditSections)
                {
                    if (section == null) continue;
                    text.AppendLine($"<b>{section.sectionTitle}</b>");
                    text.AppendLine();
                    if (section.credits != null)
                        foreach (var credit in section.credits) text.AppendLine(credit);
                    if (spaceBetweenSections) text.AppendLine();
                }
            }
            text.AppendLine("Thank You For Playing!");
            text.AppendLine();
            return new CreditContent(text.ToString(), scrollSpeed, delayBeforeStart, delayAfterEnd);
        }
    }
}
