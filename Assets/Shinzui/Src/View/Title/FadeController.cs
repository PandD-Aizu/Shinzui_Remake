...
namespace Shinzui.Src.Title
{
    public class FadeController : MonoBehaviour
    {
        ...
        private readonly Dictionary<RectTransform, Vector2> targetPositions = new();
        ...
        public void KillAll()
        {
            ...
            foreach (var target in targetPositions)
                if (target.Key != null) target.Key.anchoredPosition = target.Value;
        }

        private Vector2 GetTargetPosition(RectTransform rect)
        {
            if (!targetPositions.TryGetValue(rect, out var position))
            {
                position = rect.anchoredPosition;
                targetPositions.Add(rect, position);
            }
            return position;
        }
        ...
        private void AnimateImages()
        {
            ...
            for (int i = 0; i < fadeInObjects.Count; i++)
            {
                ...
                // 目標座標を保持
                Vector2 targetPos = GetTargetPosition(rt);
